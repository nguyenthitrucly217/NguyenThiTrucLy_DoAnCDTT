...
function ProductSaleRestore() {
    ...
    useEffect(function(){
        (async function(){
            await productsaleservice.getlistRestore(4,page).then(function(result){
    ...
    function productsaleDelete(id){
        productsaleservice.remove(id).then(function(result){
            console.log(result.data.message);
            setStatusDelete(result.data.id)
        });
    }
    function productsaleRestore(id){
        productsaleservice.restore(id).then(function(result){
            console.log(result.data.message);
            setStatusDelete(result.data.id)
        });
    }
             ...
             <div className="card-header">
                <div className="row">
                    ...
                    <div className="col-6 text-end">
                        <Link className=" btn btn-sm btn-success" to="/admin/productsale">
                           Về danh sách
                        </Link>
                    </div>
                 ...
                 <tbody>
                    {productsale.map(function(productsale,index){
                        ...
                        <tr key={index}>
                            ...
                            <td>
                            <button onClick={()=>productsaleRestore(productsale.id)} className="btn btn-sm btn-danger me-1"><FaTrashRestore/></button>
...
export default ProductSaleRestore;
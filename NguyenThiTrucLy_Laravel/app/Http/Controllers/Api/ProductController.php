...
//khôi phuc
   ...
   public function get_byPage($limit, $page = 1)  // phân trang
   { 
        $countAll = Product::where('status','!=',0)->count();
           $end_page = 1;
           if ($countAll > $limit) {
       ...
       return response()->json(
           ...
           ['success' => true, 'message' => "tai du lieu thanh cong", 'products' => $product,
           'end'=>$end_page
       ],
   ...
   { 
        $countAll = Product::where('status','!=',1)->count();
           $end_page = 1;
           if ($countAll > $limit) {
               $end_page = ceil($countAll / $limit);
           }    
